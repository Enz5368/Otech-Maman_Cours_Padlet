      ...
      function startFreeExampleTutorial() {
        ...
        activeTutorialSteps = [
          ...
          { view: "studentClasses", selector: "#content", title: "Plan de classe", text: "Chaque groupe possède un plan de classe façon salle de cinéma, réglable jusqu’à 40 sièges, avec placement libre des élèves." },
      ...
      function openSeatingPlan(groupId) {
        ...
        const modal = document.querySelector("#editorModal");
        modal.hidden = false;
        modal.innerHTML = `<section class="seating-dialog" data-group-id="${escapeAttr(group.id)}"><header class="subtree-head"><div><p>Groupe classe</p><h2>Plan de classe · ${escapeHtml(group.title)}</h2><span class="muted">Double-cliquez sur un siège pour placer un élève, puis faites glisser les sièges pour les réorganiser.</span></div><button class="btn" onclick="closeEditor()">Fermer</button></header><div class="seating-controls"><label>Rangées <input type="number" min="1" max="8" value="${plan.rows}" onchange="resizeSeatingPlan('${group.id}',this.value,null)"></label><label>Sièges par rangée <input type="number" min="1" max="10" value="${plan.columns}" onchange="resizeSeatingPlan('${group.id}',null,this.value)"></label><span class="pill">${plan.desks.length} sièges · maximum 40</span><button class="btn primary" onclick="saveSeatingPlan('${group.id}',this)">Enregistrer</button></div><div class="cinema-room"><div class="classroom-board"><span>TABLEAU</span><small>AVANT DE LA CLASSE</small></div><div class="seating-grid" style="grid-template-columns:repeat(${plan.columns},minmax(100px,1fr))">${plan.desks.map((name,index)=>`<button class="desk ${name ? "assigned" : ""}" draggable="true" data-index="${index}" onclick="selectDesk(this,event)" ondblclick="editDeskName('${group.id}',${index},this,event)" ondragstart="event.dataTransfer.setData('text/plain','${index}')" ondragover="event.preventDefault()" ondrop="dropDesk('${group.id}',${index},event)"><span>Siège ${index+1}</span><strong>${escapeHtml(name || "Libre")}</strong><small>Double-cliquez pour nommer</small></button>`).join("")}</div></div><datalist id="studentNames">${(group.students||[]).map(name=>`<option value="${escapeAttr(name)}"></option>`).join("")}</datalist></section>`;
