...
def test_enregistrement_bloque_interface_jusqu_a_confirmation() -> None:
    assert "async function saveData(message, triggerButton, explicitStudioSave = false)" in APP_JS
    ...
    assert "if (activeSaveLocks === 0 && !studioHasChanges()) return;" in APP_JS
...
def test_accroche_accepte_un_lien_video_et_seances_ont_une_valise() -> None:
    ...
    assert "function sequenceSuitcase(sequence)" in APP_JS
    assert "function suitcaseValues(source, fallback" in APP_JS
    assert "Cette valise sera automatiquement reprise dans toutes les séances" in APP_JS
    assert "Valise pédagogique de la séquence" in APP_JS
...
def test_tutoriels_pratiques_expliquent_les_actions_principales() -> None:
    ...
    assert "function startPracticalTutorial(" in APP_JS
    assert "function editorTutorialSteps(" in APP_JS
    assert "Lancer le tutoriel de l’éditeur" in APP_JS
    assert APP_JS.count('data-tour="studio-') >= 15
    assert "Mise en forme du texte" in APP_JS
...
def test_serveur_accepte_les_formats_opendocument_du_selecteur() -> None:
    ...
    assert "espace-prof-98" in INDEX
...
def test_consigne_de_diapo_est_masquable_et_reaffichable() -> None:
...
def test_plan_de_classe_style_cinema_et_emploi_du_temps_lycee() -> None:
    ...
    assert "assets/app.js?v=espace-prof-98" in INDEX
